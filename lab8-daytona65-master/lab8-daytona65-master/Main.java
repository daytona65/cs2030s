...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    ArrayList<CompletableFuture<String>> printout = new ArrayList<>();
    ...
    try {
      ...
      while (sc.hasNext()) {
        ...
        printout.add(BusSg.findBusServicesBetween(srcId, searchString)
            .thenApply(s -> s.description().join()));
      }
      sc.close();
      CompletableFuture.allOf(printout.toArray(new CompletableFuture<?>[printout.size()]))
        .thenRun(() -> printout.stream().map(x -> x.join()).forEach(System.out::println))
          .join();
