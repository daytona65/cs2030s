...
class BusAPI {
   ...
   *     query fails.
   */
  private static CompletableFuture<String> httpGet(String url) throws CompletionException {
    ...
    response = client.sendAsync(request, BodyHandlers.ofString()).handle((res, e) -> {
      if (res != null) {
        return res;
      } else {
